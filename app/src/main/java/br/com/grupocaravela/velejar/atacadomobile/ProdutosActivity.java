        ...
        try {
            ...
            if (Configuracao.getProdutoSemEstoque()){
                cursor = db
                        .rawQuery(
                                "SELECT _id, codigo, nome, estoque, expositor, valor_desejavel_venda, valor_minimo_venda, categoria_id, unidade_id, " +
                                        "ativo, peso, empresa_id, imagem, codigo_ref FROM produto ORDER BY nome COLLATE NOCASE ASC", null);
            }else{
                cursor = db
                        .rawQuery(
                                "SELECT _id, codigo, nome, estoque, expositor, valor_desejavel_venda, valor_minimo_venda, categoria_id, unidade_id, " +
                                        "ativo, peso, empresa_id, imagem, codigo_ref FROM produto WHERE estoque > '0' ORDER BY nome COLLATE NOCASE ASC", null);
            }
